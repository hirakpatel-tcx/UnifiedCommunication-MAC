...
export const ActiveCall: React.FC<ActiveCallProps> = ({
  ...
  return (
    <div className="flex-1 flex items-stretch justify-center w-full h-full p-3 sm:p-4 select-none animate-fadeIn min-h-0 overflow-hidden">
      ...
      <div className="w-full max-w-md lg:hidden rounded-3xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 shadow-2xl flex flex-col text-center transition-all relative overflow-hidden">
        {onCollapse && (
          <button
            onClick={onCollapse}
            title="Minimize"
            className="absolute top-3 right-3 p-1.5 rounded-lg text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-200 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors cursor-pointer z-10"
          >
        ...
        {onFocusCall && calls.length > 1 && (
          <div className="w-full px-4 pt-4 pb-3 border-b border-zinc-100 dark:border-zinc-800 shrink-0">
        ...
        <div className="relative flex items-center justify-center w-16 h-16 sm:w-32 sm:h-32 mb-3">
          <div className="relative z-10 flex items-center justify-center w-12 h-12 sm:w-24 sm:h-24 rounded-full bg-brand-600 text-white shadow-xl shadow-brand-500/30">
            <Volume2 className="w-5 h-5 sm:w-10 sm:h-10" />
          </div>
        </div>

        <h2 className="text-lg sm:text-3xl font-bold text-zinc-900 dark:text-zinc-100 tracking-tight">
          {parseCaller(remoteUri)}
        </h2>

        <div className="mt-2.5 inline-flex items-center gap-2 px-3 py-1 rounded-full bg-zinc-100 dark:bg-zinc-800/90 border border-zinc-200 dark:border-zinc-700/60 shadow-xs">
          <span className={`w-2 h-2 rounded-full ${statusDotClass}`} />
          <span className="text-xs sm:text-sm font-semibold text-zinc-700 dark:text-zinc-300">{statusLabel}</span>
        </div>
        </div>
      ...
      <div className="hidden lg:flex flex-col w-full max-w-4xl rounded-3xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 shadow-2xl overflow-hidden relative">
        {onCollapse && (
          <button
            onClick={onCollapse}
            title="Minimize"
            className="absolute top-4 right-4 p-1.5 rounded-lg text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-200 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors cursor-pointer z-10"
          >
            <ChevronDown className="w-4 h-4" />
          </button>
        )}
        ...
        {onFocusCall && calls.length > 1 && (
          <div className="w-full px-6 pt-6 pb-4 border-b border-zinc-100 dark:border-zinc-800">
            <LineSwitcher calls={calls} focusedCallId={callId} onSelect={onFocusCall} />
          </div>
        )}
        ...
        {/* Left: Caller info */}
        <div className="w-1/2 p-10 flex flex-col items-center justify-center text-center border-r border-zinc-200 dark:border-zinc-800">
          {isMuted && (
          ...
          <div className="relative flex items-center justify-center w-36 h-36 mb-5">
            <div className="relative z-10 flex items-center justify-center w-28 h-28 rounded-full bg-brand-600 text-white shadow-xl shadow-brand-500/30">
          ...
          <div className="mt-4 inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-zinc-100 dark:bg-zinc-800/90 border border-zinc-200 dark:border-zinc-700/60 shadow-xs">
            <span className={`w-2 h-2 rounded-full ${statusDotClass}`} />
