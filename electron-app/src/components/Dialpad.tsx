  ...
  return (
    <div className="flex-1 w-full flex flex-col select-none animate-fadeIn min-h-0 overflow-hidden">
      ...
      <div className="w-full h-full flex flex-col lg:flex-row gap-4 lg:gap-6 items-stretch min-h-0">
        ...
        <div className="w-full h-full lg:h-auto lg:w-[430px] xl:w-[460px] shrink-0 flex flex-col justify-center gap-2 sm:gap-3 p-2.5 sm:p-5 lg:p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800/80 shadow-xs overflow-y-auto">

          {/* Top: Caller ID / DID Selector */}
          <div className="relative flex items-center justify-center gap-1.5 min-h-8 sm:min-h-10 pb-2 sm:pb-3 border-b border-slate-100 dark:border-slate-800/80" ref={cidMenuRef}>
            <span className="text-[10px] sm:text-[11px] font-semibold text-slate-400 uppercase tracking-wider shrink-0">Calling as:</span>
            <button
              ref={cidBtnRef}
              onClick={() => {
                if (dids.length > 1) {
              ...
              className={`flex items-center gap-1 px-2.5 py-1 rounded-full bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-750 text-[10px] sm:text-xs text-slate-700 dark:text-slate-200 font-medium transition-colors max-w-full min-w-0 ${dids.length > 1 || dids.length === 0
                  ? 'hover:bg-slate-200 dark:hover:bg-slate-700/80 cursor-pointer'
                  : 'cursor-default'
                }`}
            >
              {activeDid ? (
                ...
                <span className="font-semibold text-slate-900 dark:text-white truncate">{callingFrom}</span>
              )}
              {dids.length > 1 && <ChevronDown className="w-3.5 h-3.5 text-slate-400 shrink-0 ml-0.5" />}
            </button>

            {/* DID / Caller ID Selection Popover */}
            {isCidMenuOpen && dids.length > 1 && (
              <FixedPopover anchorRef={cidBtnRef} align="center" className="w-[min(18rem,calc(100vw-2rem))] rounded-2xl bg-white dark:bg-[#1E2330] border border-slate-200 dark:border-slate-700 shadow-2xl overflow-hidden p-1.5">
          ...
          <div className="relative z-20 w-full">
            <div className="w-full">
              <div className="relative flex items-center bg-slate-50 dark:bg-slate-800/60 border border-slate-200/80 dark:border-slate-750 rounded-2xl p-2 sm:p-3.5 shadow-xs focus-within:bg-white dark:focus-within:bg-slate-800 focus-within:border-brand-500 focus-within:ring-2 focus-within:ring-brand-500/20 transition-all">
                <input
                  type="text"
                  value={inputNumber}
                  onChange={(e) => {
                  ...
                  className="min-w-0 w-full bg-transparent text-base sm:text-2xl lg:text-3xl font-mono text-center text-slate-900 dark:text-slate-100 placeholder:text-xs placeholder:sm:text-base placeholder:font-sans placeholder-slate-400 dark:placeholder-slate-500 outline-none px-1 sm:px-2 tracking-wide truncate"
                />
                {inputNumber && (
                  <button
                    ...
                    className="absolute right-3 p-2 text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-xl transition-colors cursor-pointer"
                  >
            ...
            {matchingContacts.length > 0 && !suggestionsDismissed && (
              <div className="absolute left-0 right-0 top-full mt-1 z-30 p-1.5 rounded-2xl bg-white dark:bg-slate-850 border border-slate-200 dark:border-slate-750 shadow-xl animate-popIn max-h-56 overflow-y-auto">
                ...
                {matchingContacts.map((contact) => (
                  ...
                  >
                    <div className="flex items-center gap-2.5 min-w-0">
                      ...
                      <div className="min-w-0">
                        <div className="text-sm font-semibold text-slate-800 dark:text-slate-200 leading-tight truncate">
                          {contact.name}
                        </div>
          ...
          <div className="grid grid-cols-3 gap-2 sm:gap-3 w-full py-1 sm:py-2 shrink-0">
            {KEYPAD_BUTTONS.map((btn) => (
              <button
                ...
                disabled={disabled}
                className="tactile-btn flex flex-col items-center justify-center h-12 sm:h-16 lg:h-17 rounded-2xl bg-slate-50 dark:bg-slate-800/70 border border-slate-200/80 dark:border-slate-750 hover:bg-white dark:hover:bg-slate-750 hover:border-brand-500/30 hover:shadow-xs active:scale-95 transition-all cursor-pointer disabled:opacity-50"
              >
                <span className="text-xl sm:text-3xl font-bold font-mono text-slate-800 dark:text-slate-100 leading-none">
                  {btn.digit}
                </span>
                {btn.subtext ? (
                  <span className="text-[8px] sm:text-[10px] font-semibold text-slate-500 dark:text-slate-400 tracking-widest uppercase mt-0.5 sm:mt-1">
                    {btn.subtext}
                  </span>
                ) : (
          ...
          <div className="shrink-0 pt-2 sm:pt-3 border-t border-slate-100 dark:border-slate-800/80 flex items-center justify-between gap-4 w-full bg-white dark:bg-slate-900">
            {lastCalledNumber ? (
              <button
                onClick={() => setInputNumber(lastCalledNumber)}
                title={`Redial ${lastCalledNumber}`}
                className="flex items-center gap-2 px-3.5 py-1.5 sm:py-2 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors cursor-pointer text-xs font-medium"
              >
                <RotateCcw className="w-4 h-4 text-slate-500" />
                <span className="font-mono text-xs hidden sm:inline">{lastCalledNumber}</span>
              </button>
            ) : (
              <div className="text-[11px] text-slate-400 font-mono">Ready to call</div>
            )}
        ...
        {activeCall ? (
          ...
          <div className="hidden lg:flex flex-1 flex-col rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800/80 shadow-xs min-h-0 overflow-hidden">
            ...
            {/* Header & Tabs */}
            <div className="px-4 pt-4 pb-3 border-b border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shrink-0">
              <div className="flex items-center gap-2">
                <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-800/80 p-1 rounded-xl border border-slate-200/60 dark:border-slate-700/60 text-xs">
                  ...
                  >
                    ...
                    <span className="px-1.5 py-0.2 rounded-full bg-slate-200 dark:bg-slate-600 text-[10px] font-mono">
                      {history.length}
                    </span>
                  </button>
            ...
            {sideTab === 'recents' ? (
              ...
              <div className="flex-1 overflow-y-auto p-3 sm:p-4 divide-y divide-slate-100 dark:divide-slate-800/80 min-h-0">
                {filteredSideContacts.length === 0 ? (
                  <div className="h-full min-h-[260px] flex flex-col items-center justify-center text-center text-slate-400 text-xs">
                    <Users className="w-10 h-10 mb-2 text-slate-300 dark:text-slate-700 stroke-[1.5]" />
                    <p className="font-semibold text-slate-600 dark:text-slate-300">No contacts found</p>
                    <p className="text-slate-400 text-[11px]">Team members and directory contacts will show up here</p>
                  </div>
                ) : (
            ...
            {/* Bottom Bar Hints */}
            <div className="p-3 border-t border-slate-100 dark:border-slate-800/80 bg-slate-50/50 dark:bg-slate-900/50 text-[11px] text-slate-400 flex items-center justify-between shrink-0">
